...
### 阶段二：加固（P1，约 2–4 周）——安全与稳定核心

| # | 事项 | 关联 | 工作量 | 验收标准 |
|---|------|------|--------|---------|
| 1 | 登录失败锁定（如 5 次锁 15 分钟）+ 密码复杂度策略 + 重置密码随机化并强制改密 | S2 | 2–3d | 锁定/复杂度/随机重置均有测试覆盖 |
| 2 | PLC 写操作审计 + 配置修改审计 + 审计拦截器化（业务无感接入） | S4 | 3d | PLC WriteAsync 留痕（操作人/地址/值/结果）；配置保存留痕；新增业务无需手工调审计 |
| 3 | 服务层权限校验入口（RecipeManager/ReportCenterService 先行） | S3 | 2d | 无权限调用服务层方法被拒绝并记审计 |
| 4 | `IReportDataProvider` 移至 `AP.Contracts.Report` + 验证插件 Provider 可被 Host 收集 | R1 | 1d | 业务插件 Provider 生成的报表真实出现在报表中心 |
| 5 | PLC 看门狗监督重启 + Scanner 断线重连（ErrorReceived + 重开策略） | T5/T6 | 2–3d | 模拟看门狗异常退出后自动恢复；USB 拔插后扫码恢复 |
| 6 | 韧性管道接线：DB 操作接 `Database-Retry`；移除误导性 Empty 注册（gRPC 部分 ❄ 冻结） | T7 | 1d | 管道调用点存在且有效 |
| 7 | 托盘重启加单实例 Mutex | T8 | 0.5d | 双击 exe/重启操作不产生双进程 |
| 8 | `AP.Infra.Security` 单元测试（哈希、登录、权限、审计）+ CI 最小流水线（build + test + 覆盖率上传） | C1/C2 | 3–5d | Security 核心路径有测试；PR 触发 CI 且失败可阻断 |
| 9 | 仓库示例连接串改为占位符（杜绝"仓库可放真实密码"的先例） | S5 | 0.5d | 仓库无明文密码 |
